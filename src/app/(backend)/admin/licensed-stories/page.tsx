...
function LicensedStoriesContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { session, loading, isAuthenticated } = useSession();

  const [stories, setStories] = useState<LicensedStory[]>([]);
  ...
  const [storyToDelete, setStoryToDelete] = useState<LicensedStory | null>(
    null
  );
  ...
  const fetchStories = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error loading licensed stories:", error);
      setError(
        "Không thể tải danh sách truyện bản quyền. Vui lòng thử lại sau."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  // Fetch stories on page load and when params change
  useEffect(() => {
    if (isAuthenticated && session?.user.role === "admin") {
      fetchStories();
    }
  }, [isAuthenticated, session, searchParams, fetchStories]);
  ...
  const handleStatusFilter = (status: string | null) => {
    ...
    router.push(`/admin/licensed-stories?${params.toString()}`);
  };
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 mb-6">
        <h1 className="text-2xl font-bold">Quản lý truyện bản quyền</h1>
        <div className="breadcrumbs text-sm">
          <ul>
            <li>
              <a>Admin</a>
            </li>
            <li>Truyện bản quyền</li>
          </ul>
        </div>
      </div>
      ...
      <div className="card bg-base-100 shadow-lg">
        <div className="card-body p-4 md:p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <h2 className="card-title">Danh sách truyện bản quyền</h2>
            <Link
              href="/admin/licensed-stories/add"
            ...
            <table className="table table-zebra">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  stories.map((story) => (
                    <tr key={story.id}>
                      <td className="hidden md:table-cell">{story.id}</td>
                      <td>
                      ...
                      <td>
                        <div className="flex gap-1">
                          <Link
                            href={`/admin/licensed-stories/${story.id}/edit`}
          ...
          <div className="mt-4 flex flex-col sm:flex-row justify-between items-center gap-2">
            <div className="text-sm text-base-content/70 order-2 sm:order-1">
              Hiển thị {stories.length} / {pagination.total || stories.length}{" "}
              kết quả
            </div>
      ...
      {storyToDelete && (
        <div className="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-60 flex items-center justify-center p-4">
          <div className="bg-base-100 p-6 rounded-box shadow-lg max-w-md w-full">
            ...
            <p>
              Bạn có chắc chắn muốn xóa truyện &ldquo;{storyToDelete.title}
...
export default function LicensedStoriesPage() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <LicensedStoriesContent />
    </Suspense>
  );
}
